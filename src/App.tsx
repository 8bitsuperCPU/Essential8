...
function AppInner({ searchOpen, setSearchOpen, search }) {
  ...
  return (<div className="min-h-screen bg-cyber-bg text-cyber-text"><Header onSearchClick={() => setSearchOpen(true)} /><Routes><Route path="/" element={<Dashboard />} /><Route path="/strategy/:strategyId" element={<StrategyDetail />} /><Route path="/strategy/:strategyId/maturity/:level" element={<MaturityView />} /><Route path="/audit" element={<AuditHome />} /><Route path="/audit/workflow/:groupId" element={<AuditWorkflowPage />} /><Route path="/audit/:auditId" element={<AuditWorkflowWrapper />} /><Route path="/audit/:auditId/report" element={<AuditReportWrapper />} /><Route path="/audit/overall-report" element={<OverallComplianceReport />} /><Route path="*" element={<Navigate to="/" replace />} /></Routes>{searchOpen && <SearchModal query={search.query} setQuery={search.setQuery} results={search.results} onClose={() => { setSearchOpen(false); search.setQuery(''); }} />}<div className="fixed bottom-3 left-3 z-30"><span className="rounded bg-cyber-card/80 border border-cyber-border px-2 py-1 text-[10px] text-cyber-muted backdrop-blur-sm">v{APP_VERSION}</span></div></div>);
}
